...
ssize_t cwsserver::OnTcpAccept(fd_t fd, const sockaddr_storage& sa, const inet::ssl_t& ssl, const std::weak_ptr<inet::cpoll>& poll) {
	ssize_t res{ -1 };
	if (auto&& self{ poll.lock() }; self) {
		return self->PollAdd(fd, EPOLLIN | EPOLLRDHUP, std::bind(&cwsserver::WsAccept, this, std::placeholders::_1, std::placeholders::_2, sa, ssl, poll));
	}
	return res;
}


ssize_t cwsserver::WsAccept(fd_t fd, uint events, const sockaddr_storage& sa, const inet::ssl_t& ssl, const std::weak_ptr<inet::cpoll>& poll) {
	/* Read HTTP request */
	ssize_t res{ -1 };
				...
				http::ToNumber(http::GetValue(headers, "sec-websocket-version")) >= 12)
			{
				if (res = WsUpgrade(so, path, args, headers, std::move(request), std::move(content)); res == 0) {
					if (auto&& con = OnWsUpgrade(so, path, args, headers, std::move(request), std::move(content)); con) {
						res = self->PollUpdate(fd, EPOLLIN | EPOLLRDHUP, std::bind(&cwsserver::WsData, this, std::placeholders::_1, std::placeholders::_2, con, poll));
						if (res != 0) {
