#include "inet/cepoll.h"

#include <sys/eventfd.h>
#include <unistd.h>

#include <csignal>
#include <functional>
#include <vector>

#include "log/clog.h"

using namespace inet;

inline void cpoll::Gc() {
    std::time_t now {std::time(nullptr)};
    while (!fdQueueGC.empty()) {
        auto obj {fdQueueGC.front()};
        fdQueueGC.pop_front();
        if (auto self {obj.lock()}; self and not self->IsLeaveUs(now)) {
            fdQueueGC.push_back(obj);
        }
        break;
    }
}

void cpoll::PollThread(const std::shared_ptr<cpoll>& self, int numEventsMax, [[maybe_unused]] int msTimeout) {
    std::vector<struct epoll_event> events_list(numEventsMax);
    pthread_setname_np(pthread_self(), self->NameOf());
    PSHT_INFO("{} ({}) start thread {}", self->NameOf(), self->fdPoll, gettid());

    try {
        sigset_t epoll_sig_mask;
        sigset_t sigset;
        sigemptyset(&sigset);
        sigaddset(&sigset, SIGPIPE);
        sigaddset(&sigset, SIGHUP);
        sigaddset(&sigset, SIGINT);
        pthread_sigmask(SIG_SETMASK, &epoll_sig_mask, nullptr);
        ssize_t nevents {0};
        std::unique_lock<decltype(fdLock)> lock(fdLock, std::defer_lock_t {});
        while (true) {
            if (nevents = epoll_wait((int)self->fdPoll, (struct epoll_event*)events_list.data(), (int)events_list.size(), 100 /*msTimeout*/); nevents > 0) {
                while (nevents-- > 0) {
                    auto& event = events_list[nevents];
                    lock.lock();
                    if (event.data.fd == self->exitFd) {
                        PSHT_INFO("{} ({}) finish thread {}", self->NameOf(), self->fdPoll, gettid());
                        return;
                    }
                    if (auto&& hFd {self->fdHandlers.find(event.data.fd)}; hFd != self->fdHandlers.end()) {
                        auto handler {hFd->second};
                        lock.unlock();
                        if (handler) {
                            handler(event.data.fd, event.events);
                            continue;
                        }
                    } else {
                        lock.unlock();
                    }
                    int fd {event.data.fd};
                    PSHT_ERROR("Unhandled socket ( {} )", fd);
                    inet::Close(fd);
                }
                self->Gc();
                continue;
            } else if (!nevents) {
                self->Gc();
                continue;
            } else if (errno == EINTR) {
                continue;
            } else {
                PSHT_ERROR("epoll error ({})", inet::GetErrorStr(-errno));
                break;
            }
        }
    } catch (std::exception& ex) {
        PSHT_ERROR("Exception ({})", ex.what());
        raise(SIGHUP);
    } catch (...) {
        auto eptr = std::current_exception();
        PSHT_ERROR("Unhandled exception");
        raise(SIGHUP);
    }
}

ssize_t cpoll::Listen(int pollSize, int pollTimeoutMs) {
    if (fdPoll > 0 and !pollThread.joinable()) {
        pollThread = std::jthread(
            [](auto&& PH1, auto&& PH2, auto&& PH3) {
                cpoll::PollThread(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2), std::forward<decltype(PH3)>(PH3));
            },
            Self(),
            pollSize,
            pollTimeoutMs);

        epoll_event ev {.events = EPOLLIN, .data = {.fd = exitFd}};
        if (auto err = epoll_ctl(fdPoll, EPOLL_CTL_ADD, exitFd, &ev) == 0 ? 0 : -errno; err != 0) {
            throw std::runtime_error(fmt::format("{}:{}", __FUNCTION__, std::strerror(err)));
        }

        return pollThread.joinable() ? 0 : -EBADE;
    }
    return -EALREADY;
}

void cpoll::Join() {
    eventfd_write(exitFd, 1);

    if (pollThread.joinable()) {
        pollThread.join();
    }

    if (exitFd > 0) {
        ::close(exitFd);
        exitFd = -1;
    }

    if (fdPoll > 0) {
        ::close(fdPoll);
        fdPoll = -1;
    }
}

ssize_t cpoll::PollUpdate(fd_t fd, uint events) {// NOLINT (can't be const)
    struct epoll_event ev {
        .events = events, .data = {.fd = fd }
    };
    return epoll_ctl(fdPoll, EPOLL_CTL_MOD, fd, &ev) == 0 ? 0 : -errno;
}

void cpoll::PollDelete(fd_t& fd) {
    if (fd > 0) {
        ::close(fd);
        // std::unique_lock<decltype(fdLock)> lock(fdLock);
        // fdHandlers.erase(fd);
        fd = -1;
    }
    /*
        std::string dump;
        for (auto&& f : fdHandlers) {
            dump.append(":").append(std::to_string(f.first)).append(" ");
        }
        printf("PollDelete #%ld %s\n", fdPoll, dump.c_str());
        */
}

cpoll::cpoll(const std::string& name) : name {name} {
    if (fdPoll = epoll_create1(EPOLL_CLOEXEC); fdPoll <= 0) {
        throw std::runtime_error(fmt::format("{}:{}", __FUNCTION__, std::strerror(errno)));
    }

    if (exitFd = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK); exitFd <= 0) {
        throw std::runtime_error(fmt::format("{}:{}", __FUNCTION__, std::strerror(errno)));
    }
}

cpoll::~cpoll() {
    Join();
}