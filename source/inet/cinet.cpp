...
ssize_t inet::ResolveAddress(const std::string& hostUrl, sockaddr_storage& sa, const std::string& defPort) {
    static const std::regex re(R"(^(\w+://)?(?:.*?@)?([^:/]+)(?::(\d+))?.*)");
    memset(&sa, 0, sizeof(sa));

    // Get address info from DNS

    if (std::cmatch match; std::regex_match(hostUrl.data(), hostUrl.data() + hostUrl.length(), match, re)) {
        if (match.str(1).compare(0, 7, "unix://") != 0) {
            ...
            if (int status = getaddrinfo(match.str(2).c_str(), nullptr, &hints, &res); status != 0) {
                return -EHOSTUNREACH;
            }
            for (it = res; it != nullptr; it = it->ai_next) {
                if (it->ai_family == AF_INET) {// IPv4
                    memcpy((void*)(&sa), (void*)it->ai_addr, sizeof(sockaddr_in));
                    ((sockaddr_in&)sa).sin_port = htons((uint16_t)(std::stoul(match.length(3) != 0 ? match[3].str() : defPort)));
                    break;
                }
                if (it->ai_family == AF_INET6) {// IPv6
                    memcpy((void*)(&sa), (void*)it->ai_addr, sizeof(sockaddr_in6));
                    ((sockaddr_in6&)sa).sin6_port = htons((uint16_t)(std::stoul(match.length(3) != 0 ? match[3].str() : defPort)));
                }
            }
            freeaddrinfo(res);
            if (sa.ss_family == 0) {
            ...
            sa.ss_family = AF_UNIX;
            strncpy(((struct sockaddr_un&)sa).sun_path, hostUrl.data(), std::min((size_t)108UL, hostUrl.length()));
        }
        return 0;
    }
...
ssize_t inet::UdpConnect(fd_t& fd, bool nonblock, int af) {
    fd = -1;
    if (fd = ::socket(af, SOCK_CLOEXEC | SOCK_DGRAM | (nonblock ? SOCK_NONBLOCK : 0), IPPROTO_UDP); fd <= 0) {
        return -errno;
    }
...
ssize_t inet::UdpConnect(fd_t& fd, const sockaddr_storage& sa, bool nonblock) {
    ssize_t res {0};
    fd = -1;
    if (fd = ::socket(sa.ss_family, SOCK_CLOEXEC | SOCK_DGRAM | (nonblock ? SOCK_NONBLOCK : 0), IPPROTO_UDP); fd <= 0) {
        return -errno;
    }
...
ssize_t inet::SslConnect(fd_t& fd, const sockaddr_storage& sa, bool nonblock, std::time_t conntimeout, const inet::ssl_ctx_t& ctx, inet::ssl_t& ssl) {
    ...
    if (ctx) {
        if (ssl = std::shared_ptr<SSL>(SSL_new(ctx.get()),
                                       [](SSL* ssl) {
                                           SSL_free(ssl);
                                       });
            !ssl) {
...
ssize_t inet::TcpServer(int& fd, const sockaddr_storage& sa, bool reuseaddress, bool reuseport, bool nonblock, int maxlisten) {
    ...
    if (int nvalue = 1; reuseaddress && setsockopt((int)fd, SOL_SOCKET, SO_REUSEADDR, &nvalue, sizeof(nvalue)) != 0) {
        res = -errno;
        ::close((int)fd);
        fd = -1;
        return res;
    }
    if (int nvalue = 1; reuseport && setsockopt((int)fd, SOL_SOCKET, SO_REUSEPORT, &nvalue, sizeof(nvalue)) != 0) {
        res = -errno;
        ::close((int)fd);
        fd = -1;
        return res;
    }
    if (::bind((int)fd, (sockaddr*)&sa, sizeof(sockaddr_storage)) != 0) {
        res = -errno;
        ::close((int)fd);
        fd = -1;
        return res;
    }
    if (::listen((int)fd, maxlisten) != 0) {
        res = -errno;
        ::close((int)fd);
        fd = -1;
        return res;
...
ssize_t inet::UdpServer(int& fd, const sockaddr_storage& sa, [[maybe_unused]] bool reuseaddress, [[maybe_unused]] bool reuseport, bool nonblock) {
    ssize_t res {0};
    fd = -1;
    if (fd = ::socket(sa.ss_family, SOCK_CLOEXEC | SOCK_DGRAM | (nonblock ? SOCK_NONBLOCK : 0), IPPROTO_UDP); fd <= 0) {
        return -errno;
        ...
        if (defaultAF != AF_UNIX) {
            ...
            if (std::regex_match(strHostPort.begin(), strHostPort.end(), match, reHostPort)) {
                if (match.length(1)) {// ip4 address found
                    ...
                    if (auto res =
                            inet_pton(AF_INET6, match.str(2).c_str(), (void*)((sockaddr_in6*)&sa)->sin6_addr.__in6_u.__u6_addr8);
                        res > 0) {
...
ssize_t inet::SslSetGlobalContext(const std::shared_ptr<SSL_CTX>& sslCtx) {
    inet::ssl::gloContext = sslCtx;// deleted move
    return 0;
}
...
ssize_t inet::SslAcceptConnection(ssize_t cli, const std::shared_ptr<SSL_CTX>& sslCtx, std::shared_ptr<SSL>& sslSocket) {
    if (sslSocket = std::shared_ptr<SSL> {SSL_new(sslCtx.get()),
                                          [](SSL* ssl) {
                                              SSL_shutdown(ssl);
                                              SSL_free(ssl);
                                          }};
...
ssize_t inet::SslCreateSelfSignedContext(std::shared_ptr<SSL_CTX>& sslCtx) {
    ...
    if (sslCtx = std::shared_ptr<SSL_CTX> {SSL_CTX_new(SSLv23_server_method()),
                                           [](SSL_CTX* ctx) {
                                               SSL_CTX_free(ctx);
                                           }};
        !sslCtx) {
    ...
    SSL_CTX_set_default_passwd_cb(sslCtx.get(), [](char* buf, int size, [[maybe_unused]] int rwflag, void* password) -> int {
...
ssize_t inet::SslCreateContext(std::shared_ptr<SSL_CTX>& sslCtx, const std::string& certFile, const std::string& privateKeyFile, bool novalidate) {
    ...
    if (!certFile.empty() && !privateKeyFile.empty()) {
        sslCtx = std::shared_ptr<SSL_CTX> {SSL_CTX_new(SSLv23_server_method()), [](SSL_CTX* ctx) {
                                               SSL_CTX_free(ctx);
                                           }};
        if (!sslCtx) {
