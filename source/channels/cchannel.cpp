...
json::value_t cchannel::ApiStats() {
    return json::object_t {{"channel", chName},
                           {"type", Str(chType)},
...
json::value_t cchannel::ApiOverview() {
    ...
    return json::object_t {{"channel", chName}, {"type", Str(chType)}, {"close", chMode == autoclose_t::yes ? "auto" : "manual"}, {"sessions", sess}};
}
...
size_t cchannel::Push(message_t&& message) {
    size_t nSubscribers {0};
    msg::delivery_t delivery {(*message)["#msg-delivery"].get<std::string_view>() == "broadcast"
                                  ? msg::delivery_t::broadcast
    ...
    [[maybe_unused]] size_t notPushed {0};
...
cchannel::cchannel(const std::shared_ptr<cchannels>& channels, const std::string& cuid, const std::string& name, const app_t& app, const channel_t& type, autoclose_t mode) :
    chType {type},
    chMode {mode},
    chUid {cuid},
    chName {name},
