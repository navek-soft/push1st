...
template <typename T>
static inline cwebsocket::msg_t MakeUnFrame(cwebsocket::opcode_t opcode, const std::string_view& message) {
...
template <typename T>
static inline cwebsocket::msg_t MakeFrame(cwebsocket::opcode_t opcode, const std::string_view& message) {
    std::vector<uint8_t> payload(sizeof(T) + message.size());
...
cwebsocket::msg_t cwebsocket::Make(cwebsocket::opcode_t opcode, const std::string_view& message, bool masked) {
    if (message.size() < 126) {
        if (!masked || message.empty()) {
            return MakeUnFrame<cwebsocket::small_un_frame_t>(opcode, message);
        } else {
            return MakeFrame<cwebsocket::small_frame_t>(opcode, message);
        }
    } else if (message.size() < 65536) {
        if (!masked) {
            return MakeUnFrame<cwebsocket::medium_un_frame_t>(opcode, message);
        } else {
            return MakeFrame<cwebsocket::medium_frame_t>(opcode, message);
        }
    } else {
        if (!masked) {
            return MakeUnFrame<cwebsocket::large_un_frame_t>(opcode, message);
        } else {
            return MakeFrame<cwebsocket::large_frame_t>(opcode, message);
        }
    }
    return {};
}
