...
ssize_t cwsconnection::WsReadMessage(size_t maxMessageLength) {
    ...
    if (websocket_t::frame_t packet; (res = WsRecv(&packet, sizeof(packet), nread, 0)) == 0) {
        ...
        if (auto length = packet.len; length < 126) {
            ...
            if ((res = WsRecv(&extra, sizeof(websocket_t::extra_t::medium_t), nread, MSG_WAITALL)) == 0) {
                nlength = be16toh(extra.medium_t.len);
            } else {
                OnWsError(res);
                return res;
            }
        } else if (length == 127) {
            if ((res = WsRecv(&extra, sizeof(websocket_t::extra_t::large_t), nread, MSG_WAITALL)) == 0) {
                nlength = be64toh(extra.large_t.len);
            } else {
                OnWsError(res);
                return res;
            }
        ...
        if (nlength) {
            if (nlength < maxMessageLength) {
                message = std::shared_ptr<uint8_t[]> {new uint8_t[nlength]};
                if ((res = WsRecv(message.get(), nlength, nread, MSG_WAITALL)) == 0) {// NOLINT
                    if (packet.mask) {
                        websocket_t::Mask((uint8_t*)message.get(), nlength, mask);
                    }
                }
            } else {
        ...
        if (auto opcode {(websocket_t::opcode_t)packet.opcode}; opcode == websocket_t::opcode_t::text or opcode == websocket_t::opcode_t::binary) {
            OnWsMessage((websocket_t::opcode_t)packet.opcode, message, nlength);
            return 0;
