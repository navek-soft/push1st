    ...
    switch (Type) {
        ...
        case auth_t::basic:
            if (!authdata.empty() and strncasecmp(authdata.data(), "Basic ", 6) == 0) {
                return http::FromBase64(authdata.substr(6)) == User + ":" + Pwd;
            }
            break;
        case auth_t::digest:
...
std::string cauthchallenge::Digest(const std::string_view& user, const std::string_view& pwd, const std::string_view& method, const std::string_view& uri) {
    return Md5(Md5(Unquote(user) + ":" + Unquote(Realm) + ":" + Unquote(pwd)) + ":" + Nonce + ":" + Md5(std::string {method} + ":" + std::string {uri}));
}
    ...
    switch (Type) {
        case auth_t::digest:
            nwrite = snprintf(buffer,
                              1024,
                              Opaque.empty() ? format_no_opaque : format_wh_opaque,
                              std::string {user}.c_str(),
                              Realm.c_str(),
                              Nonce.c_str(),
                              std::string {uri}.c_str(),
                              Digest(user, pwd, method, uri).c_str(),
                              Opaque.c_str());
...
std::string cauthchallenge::GenNonce(size_t length) {
    ...
    static constexpr char alphaNonce[] {"ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                        "abcdefghijklmnopqrstuvwxyz"
                                        "0123456789"};
    static constexpr size_t alphaSize {sizeof(alphaNonce)};
    while (length--) {
        nonce.push_back(alphaNonce[mrand48() % alphaSize]);
    }
    return nonce;
}
...
ssize_t ParseRequest(std::string_view request, std::string_view& method, uri_t& path, headers_t& headers, std::string_view& content, size_t& contentLength) {
    ...
    if (auto proto = ParserNext(request, '\n'); proto.compare(0, 5, "HTTP/") == 0 or proto.compare(0, 7, "RTSP/1.") == 0) {
        ...
        if (!uri.empty()) {
            ...
            if (uri[0] == '/') {
                path.uriPathList.emplace_back(uri.substr(0, 1));
            }
...
static inline size_t ParseNumber(std::string_view value) {
    ...
    while (!value.empty() and std::isdigit(value.front())) {
        nvalue = nvalue * 10 + value.front() - '0';
        value.remove_prefix(1);
    }
    return nvalue;
}
...
static inline std::string_view ParserNext(std::string_view& data, char symbol) {
    ...
    if (!data.empty()) {
        while (!data.empty() && data.front() != symbol) {
            data.remove_prefix(1);
        }
        from = {from.begin(), (size_t)(data.begin() - from.begin())};
        while (!from.empty() && std::isspace(from.front())) {
            from.remove_prefix(1);
        }
        while (!from.empty() && std::isspace(from.back())) {
...
std::string_view& TrimBlank(std::string_view& str) {
    ...
    while (!str.empty() && std::isblank(str.back())) {
        str.remove_suffix(1);
    }
    return str;
}
...
std::string ToBase64(const std::string_view& value) {
    ...
    if (i) {
        ...
        for (j = 0; (j < i + 1); j++) {
            ret.push_back(base64TableEncode[char_array_4[j]]);
        }
        while ((i++ < 3)) {
            ret.push_back('=');
        }
...
#define to_int32(bytes) \
    ...
    msg[value.size()] = 0x80;
    for (offset = value.size() + 1; offset < new_len; offset++) {
        msg[offset] = 0;
    }
...
#define to_hex(ch)                               \
    digest.push_back(dec2hex[((ch) >> 4) & 15]); \
    digest.push_back(dec2hex[(ch)&15]);

    to_hex((uint8_t)(h0));
    to_hex((uint8_t)(h0 >> 8));
    to_hex((uint8_t)(h0 >> 16));
    to_hex((uint8_t)(h0 >> 24));
    to_hex((uint8_t)(h1));
    to_hex((uint8_t)(h1 >> 8));
    to_hex((uint8_t)(h1 >> 16));
    to_hex((uint8_t)(h1 >> 24));
    to_hex((uint8_t)(h2));
    to_hex((uint8_t)(h2 >> 8));
    to_hex((uint8_t)(h2 >> 16));
    to_hex((uint8_t)(h2 >> 24));
    to_hex((uint8_t)(h3));
    to_hex((uint8_t)(h3 >> 8));
    to_hex((uint8_t)(h3 >> 16));
    to_hex((uint8_t)(h3 >> 24));
