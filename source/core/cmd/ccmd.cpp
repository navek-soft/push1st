...
extern char* program_invocation_name;      // NOLINT
extern char* program_invocation_short_name;// NOLINT
...
void ccmdline::operator()(int argc, char* argv[]) {
    ...
    for (auto&& o : optionsList) {
        ...
        if (it->val = (unsigned char)std::get<1>(o.second); it->val) {
...
std::string_view ccmdline::Program() const {// NOLINT (static)
    return program_invocation_short_name;
}

std::string_view ccmdline::Pwd() const {// NOLINT (static)
    return {program_invocation_name, static_cast<size_t>(program_invocation_short_name - program_invocation_name)};
}

size_t ccmdline::PId() const {// NOLINT (static)
    return (size_t)getpid();
}

static inline std::string_view CmdlineTrimLine(const std::string_view& v) {
...
static inline std::vector<std::string> CmdlineSplitHelp(const char* usage_help) {
    /* explode chains */
    ...
    if (usage_help) {
        while (end != std::string_view::npos) {
            list.emplace_back(
                CmdlineTrimLine({string.data() + start, static_cast<size_t>((string.data() + end) - (string.data() + start))}));
            start = end + 1;
            end = string.find('\n', start);
        }
        list.emplace_back(CmdlineTrimLine({string.data() + start, string.length() - start}));
    }
    return list;
}
...
void ccmdline::Print(const std::string& banner) const {
    ...
    for (auto&& opt : optionsList) {
        ...
        lines.emplace(!usage_section ? "" : usage_section, std::make_tuple(longname, shortname, CmdlineSplitHelp(usage_help)));
    }
    ...
    for (auto&& [s, h] : lines) {
        ...
        if (!longname.empty()) {
            printf("--%-*s\t", static_cast<int>(ln_max_size), longname.c_str());
        } else {
            printf("  %*s \t", static_cast<int>(ln_max_size), " ");
        }
        if (help_lines.size() == 1) {
            ...
            for (size_t i = 1; i < help_lines.size(); ++i) {
                printf("\n%*s\t%s", static_cast<int>(line_num_spaces), " ", help_lines[i].c_str());
            }
        }
        printf("\n");
    }
