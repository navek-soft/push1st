...
static inline std::string PathValue(const yaml_t& vals, const std::string& def) {
    ...
    while (!trimPath.empty() and trimPath.front() == '/') {
        trimPath.remove_prefix(1);
    }
    while (!trimPath.empty() and trimPath.back() == '/') {
        trimPath.remove_suffix(1);
    }

    return !trimPath.empty() ? std::string {trimPath} : def;
}

void cconfig::server_t::Load([[maybe_unused]] const std::filesystem::path& path, const yaml_t& options) {
    if (options.IsDefined() and options.IsMap()) {
        ...
        if (options["websocket"].IsMap()) {
            ...
            WebSocket.PushOn = Map<channel_t::type>(options["websocket"]["push"], {{"public", channel_t::type::pub}, {"private", channel_t::type::priv}, {"presence", channel_t::type::pres}}, channel_t::type::pub | channel_t::type::prot | channel_t::type::pres);
        ...
        if (options["mqtt"].IsMap()) {
            ...
            MQTT.PushOn = Map<channel_t::type>(options["mqtt"]["push"], {{"public", channel_t::type::pub}, {"private", channel_t::type::priv}, {"presence", channel_t::type::pres}}, channel_t::type::pub | channel_t::type::prot | channel_t::type::pres);
...
void cconfig::interface_t::smpp_t::Load([[maybe_unused]] const std::filesystem::path& path, const yaml_t& options) {
...
cconfig::credential_t::credential_t(const credential_t& cred) :
    ...
    Hooks {(cred.Hooks)},
    OptionClientMessages {cred.OptionClientMessages},
    OptionStatistic {cred.OptionStatistic},
    Origins {(cred.Origins)},
    ...
    if (credOptions.IsDefined() and credOptions.IsMap()) {
        for (auto&& cred : credOptions) {
            ...
            if (cred.second["origins"].IsDefined() and cred.second["origins"].IsSequence()) {
                for (auto&& orig : cred.second["origins"]) {
                    Origins.emplace(orig.as<std::string>());
                }
            }
            ...
            if (cred.second["hook"].IsDefined()) {
                ...
                if (cred.second["hook"]["trigger"].IsSequence()) {
                    Hooks = Map(cred.second["hook"]["trigger"], {{"register", hook_t::type::reg}, {"unregister", hook_t::type::unreg}, {"join", hook_t::type::join}, {"leave", hook_t::type::leave}, {"push", hook_t::type::push}}, hook_t::type::none);
                    if (Hooks != hook_t::type::none) {
                        if (cred.second["hook"]["endpoint"].IsSequence()) {
                            for (auto&& item : cred.second["hook"]["endpoint"]) {
                                if (auto endpoint {item.as<std::string>()}; (endpoint.compare(0, 7, "http://") == 0 or endpoint.compare(0, 8, "https://") == 0 or endpoint.compare(0, 6, "lua://") == 0)) {
                                    Endpoints.emplace(endpoint);
                                }
                            }
                        } else if (cred.second["hook"]["endpoint"].IsScalar()) {
                            if (auto endpoint {cred.second["hook"]["endpoint"].as<std::string>()}; (endpoint.compare(0, 7, "http://") == 0 or endpoint.compare(0, 8, "https://") == 0 or endpoint.compare(0, 6, "lua://") == 0)) {
                                Endpoints.emplace(endpoint);
                            }
                        }
                    }
...
void cconfig::clogger_t::Load([[maybe_unused]] const std::filesystem::path& basedir, const yaml_t& options) {
    if (options.IsDefined() and options.IsMap()) {
        if (auto&& sinkOut = options["stdout"]; sinkOut and sinkOut.IsMap()) {
            stdoutSink = cstdout_t {.tag = "", .verbose = 4, .pattern = ""};
            clogger_t::BaseInit(stdoutSink, sinkOut);
        }

        if (auto&& sinkFile = options["file"]; sinkFile and sinkFile.IsMap()) {
            fileSink = cfile_t {.tag = "",
                                .number = sinkFile["number"].as<int>(),
                                .rotate = sinkFile["rotate"].as<int>(),
                                .verbose = 4,
                                .path = sinkFile["path"].as<std::string>(),
                                .pattern = ""};
            clogger_t::BaseInit(fileSink, sinkFile);
        }

        if (auto&& sinkSys = options["syslog"]; sinkSys and sinkSys.IsMap()) {
            syslogSink = csyslog_t {.tag = "", .verbose = 4, .facility = sinkSys["facility"].as<std::string>(), .pattern = ""};
            clogger_t::BaseInit(syslogSink, sinkSys);
        }

        if (auto&& remoteSys = options["remote"]; remoteSys and remoteSys.IsMap()) {
            remoteSink = cremote_t {.tag = "", .verbose = 4, .url = remoteSys["url"].as<std::string>(), .pattern = ""};
            clogger_t::BaseInit(remoteSink, remoteSys);
        }
    } else {
        stdoutSink = cstdout_t {.tag = "", .verbose = 4, .pattern = ""};
    }
...
bool cconfig::cdsn::Assign(const std::string& dsn) {
    ...
    if (std::regex_match(src.begin(), src.end(), match, re)) {
        Proto_ = std::string_view {match[1].first, (size_t)match[1].length()};
        if (strncasecmp(Proto_.data(), "http://", 7) == 0 or strncasecmp(Proto_.data(), "https://", 8) == 0
            or strncasecmp(Proto_.data(), "tcp://", 6) == 0 or strncasecmp(Proto_.data(), "udp://", 6) == 0) {
            User_ = std::string_view {match[3].first, (size_t)match[3].length()};
            Pwd_ = std::string_view {match[4].first, (size_t)match[4].length()};
            HostPort_ = std::string_view {match[5].first, (size_t)match[5].length()};
            Host_ = std::string_view {match[6].first, (size_t)match[6].length()};
            Port_ = std::string_view {match[7].first, (size_t)match[7].length()};
            Url_ = std::string_view {match[8].first, (size_t)match[8].length()};
            Path_ = std::string_view {match[9].first, (size_t)match[9].length()};
            Args_ = std::string_view {match[10].first, (size_t)match[10].length()};
        } else {
            Path_ = std::string_view {match[2].first, (size_t)match[2].length()};
            Args_ = std::string_view {match[10].first, (size_t)match[10].length()};
        }
        return true;
    }
