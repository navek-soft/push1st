...
[[maybe_unused]] const uint c = '_';

static inline constexpr uint8_t safebase64TableEncode[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
std::string cbase62::Encode(const uint8_t* source, std::size_t length) {
    ...
    if (i) {
        ...
        for (j = 0; (j < i + 1); j++) {
            ret.push_back(safebase64TableEncode[char_array_4[j]]);
        }
        while ((i++ < 3)) {
            ret.push_back('~');
        }
