    ...
    if (auto&& L {S()}; L) {
        if (!luaScript.empty()) {
            if (luaL_dofile(l, luaScript.c_str()) != 0) {
                PSHT_ERROR("execute(`{}`): {}", luaScript.c_str(), lua_tostring(l, -1));
                lua_pop(l, 1);
                return {};
            }
        }
            ...
            try {
                if (lua_getglobal(l, luaFunction.c_str()); lua_isfunction(l, -1)) {
                    ...
                    if (lua_pcall(l, (int)luaArgs.size(), LUA_MULTRET, 0) == 0) {
                        ...
                        PSHT_ERROR("execute(error running function `{}`): {}", luaFunction.c_str(), lua_tostring(l, -1));
                        return {};
                    }
                } else {
                    PSHT_ERROR("execute(error running function `{}`): {}", luaFunction.c_str(), lua_tostring(l, -1));
                ...
                PSHT_ERROR("exception(error running function `{}`): {}", luaFunction.c_str(), lua_tostring(l, -1));
            }
        }
    } else {
        PSHT_ERROR("execute(error script): {}", "invalid lua context (script is empty)");
    }
    return multiReturn;
}
...
template <typename T>
void push_integer(clua::lua_t L, const std::any& val) {
    lua_pushinteger(L, std::any_cast<T>(val));
}
template <typename T>
void push_number(clua::lua_t L, const std::any& val) {
    lua_pushnumber(L, std::any_cast<T>(val));
}
void push_bool(clua::lua_t L, const std::any& val) {
    lua_pushboolean(L, std::any_cast<bool>(val));
}
void push_string(clua::lua_t L, const std::any& val) {
    lua_pushstring(L, std::any_cast<const char*>(val));
}
void push_cstring(clua::lua_t L, const std::any& val) {
    auto&& _v = std::any_cast<std::string>(val);
    lua_pushlstring(L, _v.data(), _v.length());
}
void push_vstring(clua::lua_t L, const std::any& val) {
    auto&& _v = std::any_cast<std::string_view>(val);
    lua_pushlstring(L, _v.data(), _v.length());
}
void push_rawdata(clua::lua_t L, const std::any& val) {
    auto&& _v = std::any_cast<std::vector<uint8_t>>(val);
    lua_pushlstring(L, (const char*)_v.data(), _v.size());
}
void push_nil(clua::lua_t L, [[maybe_unused]] const std::any& val) {
    lua_pushnil(L);
}
...
void push_value(clua::lua_t L, const std::any& val) {
    if (val.has_value()) {
        if (auto&& it = typecast::PushTypecast.find(val.type().hash_code()); it != typecast::PushTypecast.end()) {
            ...
            PSHT_DEFAULT_ERROR("typecast({}) no found", val.type().name());
        }
    } else {
        push_nil(L, {nullptr});
    }
