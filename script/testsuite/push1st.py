...
class cwebsocket:
    ...
    async def wsthread(self, Url, callback):
        Fd = websocket.WebSocketApp(Url,on_message=callback,on_open=self.wsOnOpen)
        Fd.run_forever()

    def Subscribe(self, channels, callback):
        self.loop.run_until_complete(cwebsocket.wsthread(self,'{}/{}/{}'.format(self.UrlBase,"/".join(channels),self.UrlArgs if not self.UrlArgs else ''), callback))
    
    async def GetSubscribe(self, channels, callback):
        await cwebsocket.wsthread(self,'{}/{}/{}'.format(self.UrlBase,"/".join(channels),self.UrlArgs if not self.UrlArgs else ''), callback)
