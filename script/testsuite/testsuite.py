...
class Test_Pusher(unittest.TestCase):
    ...
    def Subscribe(self, channel, event, user_data=None, secret=Secret):
        ...
        if pus.IsConnected:
            ...
            while n >= 0 and self.IsTriggered == False:
                ...
                time.sleep(1)
            if self.IsTriggered:
                print('{:>10}::{:.<40} pass ({})'.format('PUSHER','Event',event))
        pus.Disconnect()
    ...
    def test_ChannelNoSecret(self):
        IsSubscribe, IsReceive = self.Subscribe("private-mychannel.no-secret","myevent", secret="")
        self.assertTrue(IsSubscribe == True and IsReceive == False)
