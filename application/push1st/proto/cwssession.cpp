...
message_t cwssession::UnPack(data_t&& data) {
    try {
        if (auto msg {msg::Unserialize(data, subsId)}; msg) {
            auto&& root {*msg};
            if (root.is_object() and root.contains("channel") and root.contains("event") and root.contains("data")
                and !root["channel"].empty() and root["channel"].size() <= MaxChannelNameLength) {
                return msg;
            }
        }
        PSHT_ERROR("RAW {} message ( {} )", Id().c_str(), "Invalid message format");
    } catch (std::exception& ex) { PSHT_ERROR("RAW {} Message ( {} )", Id().c_str(), ex.what()); }
    return {};
}

void cwssession::OnWsMessage([[maybe_unused]] websocket_t::opcode_t opcode, const std::shared_ptr<uint8_t[]>& data, size_t length) {
    try {
            ...
            if (auto&& chIt {SubscribedTo.find((*message)["channel"].get<std::string>())}; chIt != SubscribedTo.end()) {
                if (std::shared_ptr<cchannel> ch {chIt->second.lock()}; ch) {
                    if (EnablePushOnChannels & ch->Type()) {
                        ...
                        App->Trigger(ch->Type(), hook_t::type::push, (*message)["channel"].get<std::string>(), Id(), std::move(*message));// NOLINT
                        PSHT_INFO("RAW {} Push ( {}/{} )",
                                  Id().c_str(),
                                  (*message)["channel"].get<std::string>().c_str(),
                                  (*message)["event"].get<std::string>().c_str());
                    } else {
                        PSHT_WARNING("RAW {} Push ( {} ) disable by config",
                                     Id().c_str(),
                                     (*message)["channel"].get<std::string>().c_str(),
                                     (*message)["event"].get<std::string>().c_str());
                    }
                    ...
                    PSHT_ERROR("{} ( {} )", __PRETTY_FUNCTION__, std::strerror(EBADSLT));
                }
            } else {
                PSHT_WARNING("RAW {} Push ( {} ) no channel subscription",
                             Id().c_str(),
                             (*message)["channel"].get<std::string>().c_str(),
                             (*message)["event"].get<std::string>().c_str());
            }
        }
    } catch (std::exception& ex) { PSHT_ERROR("{} ( {} )", __PRETTY_FUNCTION__, ex.what()); }
...
[[maybe_unused]] static inline std::pair<std::string_view, std::string_view> ExplodePathName(std::string_view Path) {
...
bool cwssession::OnWsConnect(const http::uri_t& path, const http::headers_t& headers) {
    ...
    if (!nchannels) {
        PSHT_ERROR("RAW:{}:{} No channels subscriptions ( or invalid authorization )", Fd(), Id().c_str());
...
cwssession::cwssession(const std::shared_ptr<cchannels>& channels, const app_t& app, const inet::csocket& fd, size_t maxMessageLength, const channel_t& pushOnChannels, std::time_t keepAlive, const std::string& sessPrefix) :
    ...
    csubscriber {GetAddress(), GetPort(), sessPrefix},
