            ...
            if (auto chType {ChannelType(chName)}; chType == channel_t::type::priv and (*message)["data"]["auth"].is_string()) {
                if (App->IsAllowChannelSession(chType, Id(), chName, (*message)["data"]["auth"].get<std::string>())) {
                    if (auto&& chSelf {Channels->Register(chType, App, std::string {chName})}; chSelf) {
                        ...
                        if (res = WsWriteMessage(opcode_t::text, json::Serialize({{"event", "pusher_internal:subscription_succeeded"}, {"channel", chName}})); res == 0) {
                            return;
                        }
                    }
                }
                ...
                SessionPresenceData =
                    (*message)["data"]["channel_data"].is_string() ? (*message)["data"]["channel_data"].get<std::string>() : std::string {};
                if (App->IsAllowChannelSession(chType, Id(), chName, (*message)["data"]["auth"].get<std::string>(), SessionPresenceData)) {
                    if (auto&& chSelf {Channels->Register(chType, App, std::string {chName})}; chSelf) {
                        if (json::value_t ui; json::Unserialize(SessionPresenceData, ui) and ui["user_id"].is_string()) {
                            SessionUserId = ui["user_id"].get<std::string>();
                        }
                        ...
                        if (res = WsWriteMessage(
                                ...
                                json::Serialize(
                                    {{"event", "pusher_internal:subscription_succeeded"},
                                     ...
                                     {"data",
                                      json::Serialize(
                                          {{"presence", json::object_t {{"ids", ids}, {"hash", users}, {"count", users.size()}}}})}}));
                            res == 0) {
                            return;
                        }
                    }
...
void cpushersession::OnPusherPing([[maybe_unused]] const message_t& message) {
    ssize_t res;
    if (res = WsWriteMessage(opcode_t::text, json::Serialize({{"event", "pusher:pong"}})); res == 0) {
        ActivityCheckTime = std::time(nullptr) + KeepAlive;
    }

    PSHT_INFO("Ping {} ( {} )", Id().c_str(), strerror(-(int)res));
}
...
void cpushersession::OnPusherPush(const message_t& message) {
    if (auto&& chName {(*message)["channel"].get<std::string>()}; !chName.empty()) {
        ...
        PSHT_TRACE("Channel {} `{}` PULL", Id().c_str(), chName.c_str());
        return;
    }
    WsError(close_t::ProtoError, -EPROTO);
}

void cpushersession::OnWsMessage([[maybe_unused]] websocket_t::opcode_t opcode, const std::shared_ptr<uint8_t[]>& data, size_t length) {
    ActivityCheckTime = std::time(nullptr) + KeepAlive;
...
void cpushersession::OnSocketError(ssize_t err) {
    PSHT_ERROR("Session {} ( {} )", Id().c_str(), std::strerror((int)-err));
...
bool cpushersession::OnWsConnect([[maybe_unused]] const http::uri_t& path, [[maybe_unused]] const http::headers_t& headers) {
    PSHT_TRACE("Connect {}:{}", Fd(), Id().c_str());

    SetSendTimeout(500);
    SetKeepAlive(true, 2, 2, 2);

    if (WsWriteMessage(opcode_t::text,
                       json::Serialize({{"event", "pusher:connection_established"},
                                        {"data",
                                         json::Serialize({
                                             {"socket_id", Id()},
                                             {"activity_timeout", std::to_string(KeepAlive)},
                                         })}}),
                       false)
        == 0) {
...
cpushersession::cpushersession(const std::shared_ptr<cchannels>& channels, const app_t& app, const inet::csocket& fd, size_t maxMessageLength, const channel_t& pushOnChannels, std::time_t keepAlive) :
    ...
    Channels {channels},
    App {app},
    EnablePushOnChannels {pushOnChannels} {
    ActivityCheckTime = std::time(nullptr) + KeepAlive;
    // PSHT_INFO("{}", __PRETTY_FUNCTION__);
}

cpushersession::~cpushersession() {
    PSHT_TRACE("Destroy {}", Id().c_str());
    for (auto&& it : SubscribedTo) {
        if (auto&& ch {it.second.lock()}; ch) {
            ch->UnSubscribe(subsId);
        }
...
inline message_t cpushersession::UnPack(const std::shared_ptr<uint8_t[]>& data, size_t length) {
    ...
    } catch (std::exception& ex) { PSHT_ERROR("Raw {} Message ( {} )", Id().c_str(), ex.what()); }
    return {};
}