...
class ccredentials : public std::enable_shared_from_this<ccredentials> {
    class capplication : public config::credential_t {
       ...
       public:
        capplication(const std::shared_ptr<cibroker>& broker, const config::credential_t& app);
        ...
        inline bool IsAllowChannelSession(channel_t::type type, const std::string& session, const std::string_view& channel, const std::string_view& token, const std::string& data = {}, const std::string& origin = {}) {
            return (Channels & type) and (Origins.empty() or Origins.count(origin)) and ((type == channel_t::type::pub) or ValidateSession(token, session, std::string {channel}, data, origin));// NOLINT
        }
        inline bool IsAllowChannelToken(channel_t::type type, const std::string& session, const std::string_view& channel, const std::string_view& token, const std::string& data = {}, const std::string& origin = {}) {
            return (Channels & type) and (Origins.empty() or Origins.count(origin)) and ((type == channel_t::type::pub) or ValidateToken(token, session, std::string {channel}, data, origin));// NOLINT
        }
   ...
   public:
    ...
    ccredentials(const std::shared_ptr<cibroker>& broker, config::credentials_t& creds);
    ~ccredentials();
