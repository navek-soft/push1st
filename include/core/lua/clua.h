   ...
   public:
    ...
    class class_t {
       public:
        struct self_t {
            ...
            self_t(const self_t& f) {
                selfObject = f.selfObject;
                f.selfObject = nullptr;
            }
            ~self_t() {
                if (selfObject) {
                    delete selfObject;
    ...
    class engine : public std::enable_shared_from_this<engine> {
       public:
        ...
        inline void luaAddBasePackagePath(const std::string& path) {
            if (!path.empty()) {
                packageBasePath = path.back() == '/' ? path : (path + '/');
            }
        }
    ...
    class argslist {
       ...
       public:
        ...
        inline std::any string(int n) {
            if (lua_type(jitLua, n + argOffset) == LUA_TSTRING) {
                return std::string {luaL_checkstring(jitLua, n + argOffset)};
            }
            return std::string {};
        }
        ...
        inline std::any boolean(int n) {
            if (lua_type(jitLua, n + argOffset) == LUA_TBOOLEAN) {
                return (bool)lua_toboolean(jitLua, n + argOffset);
            }
            return false;
        }
        inline std::any number(int n) {
            if (lua_type(jitLua, n + argOffset) == LUA_TNUMBER) {
                return ssize_t {lua_tointeger(jitLua, n + argOffset)};
            }
            return 0;
        }
