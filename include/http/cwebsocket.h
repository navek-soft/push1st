...
#pragma pack(push, 1)
    ...
    struct frame_t {
        ...
        frame_t(cwebsocket::opcode_t op, size_t length, bool have_mask = true, bool is_fin = true) :
            opcode {(uint8_t)((int)op & 0xf)},
            rsv3 {0},
            rsv2 {0},
            rsv1 {0},
            fin {is_fin},
            len {(uint8_t)(length & 0x7f)},
            mask {have_mask} {}
    };
    ...
    struct small_un_frame_t : frame_t {
        uint8_t payload[0];
        small_un_frame_t(cwebsocket::opcode_t op, size_t length, bool is_fin = true) :
            frame_t {op, length, false, is_fin} {
            ;
        }
    };
    struct medium_frame_t : frame_t {
        ...
        mask_t key;
        uint8_t payload[0];
        medium_frame_t(cwebsocket::opcode_t op, size_t length, bool is_fin = true) :
            frame_t {op, 126, true, is_fin},
            elen {htobe16((uint16_t)length)} {
            ;
        }
    };
    struct medium_un_frame_t : frame_t {
        uint16_t elen;
        uint8_t payload[0];
        medium_un_frame_t(cwebsocket::opcode_t op, size_t length, bool is_fin = true) :
            frame_t {op, 126, false, is_fin},
            elen {htobe16((uint16_t)length)} {
            ;
        }
    };
    struct large_frame_t : frame_t {
        ...
        mask_t key;
        uint8_t payload[0];
        large_frame_t(cwebsocket::opcode_t op, size_t length, bool is_fin = true) :
            frame_t {op, 127, true, is_fin},
            elen {htobe64((uint64_t)length)} {
            ;
        }
    };
    struct large_un_frame_t : frame_t {
        uint64_t elen;
        uint8_t payload[0];
        large_un_frame_t(cwebsocket::opcode_t op, size_t length, bool is_fin = true) :
            frame_t {op, 127, false, is_fin},
            elen {htobe64((uint64_t)length)} {
            ;
        }
    };
