...
class curi {
   ...
   public:
    ...
    curi(const curi& u) :
        uriFull {u.uriFull},
        uriPath {u.uriPath},
        uriPathList {u.uriPathList.begin(), u.uriPathList.end()},
        uriArgs {u.uriArgs.begin(), u.uriArgs.end()} {
        ;
    }
    inline std::string_view At(size_t idx) const {
        return uriPathList[idx];
    }
    ...
    inline auto Args(const std::string_view& name) const {
        return uriArgs.equal_range(name);
    }
...
class cauthchallenge {
   public:
    ...
    inline void Reset() {
        Stale = false;
        Type = auth_t::none;
        Realm = Domain = Nonce = Opaque = Qop = User = Pwd = Response = {};
        Algorithm = "md5";
    }
    ...
    inline bool Empty() {
        return Type == auth_t::none;
    }

    static std::string GenNonce(size_t length);
...
inline std::string_view GetValue(const CONTAINER_T& headers, const std::string_view& header) {
    if (auto&& hIt {headers.find(header)}; hIt != headers.end()) {
        return hIt->second;
    }
    return {""};
}
template <typename CONTAINER_T>
inline std::string_view GetValue(const CONTAINER_T& headers, const std::string_view& header, const std::string_view& def) {
    if (auto&& hIt {headers.find(header)}; hIt != headers.end()) {
        return hIt->second;
    }
    return def;
...
inline bool IsConnectionKeepAlive(const headers_t& headers) {
    if (auto&& hIt {headers.find("connection")}; hIt != headers.end()) {
        return strncasecmp(hIt->second.data(), "keep-alive", 10);
    }
