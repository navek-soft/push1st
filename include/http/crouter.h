...
class crouter {
    struct key_t {
        ...
        key_t(key_t&& k) noexcept :
            route {std::move(k.route)},
            path {std::move(k.path)},
            callback {std::move(k.callback)} {
            ;
        }
        explicit key_t(const std::string& r, routefn_t&& fn) : route {r}, callback {fn} {
            path = Explode(route);
        }
        explicit key_t(const std::string_view& r) : path {Explode(r)} {
            ;
        }
        ...
        inline bool Compare(const std::vector<std::string_view>& r, std::vector<std::string_view>& args) const {
            for (size_t n {1}; n < path.size(); ++n) {
                if (path[n] == "?") {
                    args.emplace_back(n < r.size() ? r[n] : std::string_view {});
                    continue;
                }
                if (path[n] == "*") {
        ...
        static inline std::vector<std::string> Explode(std::string_view route) {
            ...
            if (!route.empty()) {
                if (route.front() == '/') {
                    path.emplace_back(route.data(), 1);
