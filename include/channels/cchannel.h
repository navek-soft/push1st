...
class cchannel {
   public:
    cchannel(const std::shared_ptr<cchannels>& channels, const std::string& cuid, const std::string& name, const app_t& app, const channel_t& type, autoclose_t mode);
    virtual ~cchannel();
    ...
    virtual size_t Push(message_t&& message);
    virtual inline void GetUsers(usersids_t&, userslist_t&) = 0;
    inline channel_t::type Type() {
        return chType;
    }
    inline size_t CountSubscribers() {
        return chSubscribers.size();
    }
    size_t Gc();
    virtual inline void OnClusterJoin([[maybe_unused]] const json::value_t& val) {
        ;
    }
    virtual inline void OnClusterLeave([[maybe_unused]] const json::value_t& val) {
        ;
    }
    virtual inline void OnClusterPush(message_t&& msg) {
        cchannel::Push(std::move(msg));
    }
...
static inline channel_t ChannelType(const std::string_view& Name) {
    if (Name.compare(0, 8, "private-") == 0) {
        return channel_t::type::priv;
    }
    if (Name.compare(0, 9, "presence-") == 0) {
        return channel_t::type::pres;
    }
    if (Name.compare(0, 10, "protected-") == 0) {
        return channel_t::type::prot;
    }
